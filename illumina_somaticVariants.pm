...
sub runSomaticVariantCallers {
    ...
    my $sample_tumor_out_dir = catfile($opt{OUTPUT_DIR}, "somaticVariants", $sample_tumor_name);
    my $sample_tumor_tmp_dir = catfile($sample_tumor_out_dir, "tmp");
    my $sample_tumor_log_dir = catfile($sample_tumor_out_dir, "logs");
    ...
    my $sample_ref_bam = catfile($opt{OUTPUT_DIR}, $sample_ref, "mapping", $opt{BAM_FILES}->{$sample_ref});
    ...
    my $invcf;
    my $outvcf = catfile($sample_tumor_out_dir, "${sample_tumor_name}_merged_somatics.vcf");
    ...
    if ($opt{SOMVAR_TARGETS}) {
        $invcf = $outvcf;
        $outvcf = catfile($sample_tumor_out_dir, "${sample_tumor_name}_filtered_merged_somatics.vcf");
    ...
    $outvcf =~ s/\.vcf/$suffix/;
    print MERGE_SH "$opt{OUTPUT_DIR}/scripts/melt_somatic_vcf.py -t $sample_tumor -v $invcf > $outvcf\n\n";
...
sub runVarscan {
    ...
    my $varscan_out_dir = catfile($out_dir, "varscan");
...
sub runFreeBayes {
    ...
    foreach my $chr (@chrs) {
        ...
        $freebayes_command .= " -f $opt{GENOME} -r $chr $opt{FREEBAYES_SETTINGS} $sample_ref_bam $sample_tumor_bam > $freebayes_out_dir/$output_name.vcf";
