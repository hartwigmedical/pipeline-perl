...
SKIP: {
    ...
    $sample = HMF::Pipeline::Config::Validate::verifyBam(
        $bam_path, {
    ...
    is(
        HMF::Pipeline::Config::Validate::verifyBai(
            "${bam_path}.bai",
            $bam_path, {
                SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH},
                GENOME => catfile("t", "data", "empty.fasta"),
    ...
    ok(HMF::Pipeline::Config::Validate::verifyFlagstat($flagstat_path, $bam_path), "validates flagstat");

    # required for stupid filesystem timestamp granularity
    sleep 1;
    touch($bam_path);
    ok(!HMF::Pipeline::Config::Validate::verifyFlagstat($flagstat_path, $bam_path), "detects flagstat too old");

    unlink($flagstat_path);
    ok(!HMF::Pipeline::Config::Validate::verifyFlagstat($flagstat_path, $bam_path), "detects flagstat missing");

    $bam_path = Util::convertToBam(catfile("t", "data", "sample.sam"));
    $headers = HMF::Pipeline::Config::Validate::bamHeaders($bam_path, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}});
    is_deeply(
        $headers, [ {
                name => '@SQ',
                tags => {SN => 1, LN => 0}
    ...
    $reads = HMF::Pipeline::Config::Validate::bamReads($bam_path, 100, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}});
    is_deeply(
        $reads, [ {
                qname => "seq",
                flag => "0",
    ...
    $reads = HMF::Pipeline::Config::Validate::bamReads($bam_path, 1, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}});
    is_deeply(
        $reads, [ {
                qname => "seq",
                flag => "0",
    ...
    my $contigs = HMF::Pipeline::Config::Validate::indexContigs($bam_path, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}});
    is_deeply(
        $contigs, {
            1 => "2",
            2 => "3",
    ...
    my $contig_lengths = HMF::Pipeline::Config::Validate::contigLengths(
        [
            "1	123\n", #
            "2	456\n", #
            "3	789",   #
    ...
    warnings_are {
        $exception = exception {
            $result = HMF::Pipeline::Config::Validate::verifyContigs({
                    1 => "42",
                    2 => "420",
                }, {
                    1 => "42",
    ...
    warnings_are {
        $exception = exception {
            HMF::Pipeline::Config::Validate::verifyContigs({
                    1 => "42",
                    2 => "420",
                }, {
                    1 => "42",
    ...
    $exception = exception {
        $result = HMF::Pipeline::Config::Validate::verifyReadGroups(
            [
                {tags => {ID => "id1"}},        #
                {tags => {ID => "id2"}},        #
            ], [
    ...
    $exception = exception { my $sample = HMF::Pipeline::Config::Validate::verifyBam($temp_file->filename, {}) };
    like($exception, qr/^ERROR: $temp_file does not exist/, "detects missing BAM for verification");

    stderr_isnt {
        $exception = exception { $headers = HMF::Pipeline::Config::Validate::bamHeaders($temp_file, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}}) }
    }
    ...
    stderr_isnt {
        $exception = exception { $reads = HMF::Pipeline::Config::Validate::bamReads($temp_file, 1, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}}) }
    }
    ...
    stderr_isnt {
        $exception = exception { $reads = HMF::Pipeline::Config::Validate::indexContigs($temp_file, {SAMTOOLS_PATH => $ENV{SAMTOOLS_PATH}}) }
    }
    ...
    stderr_isnt {
        $exception = exception { $reads = HMF::Pipeline::Config::Validate::refGenomeContigs({GENOME => $temp_file->filename}) }
    }
