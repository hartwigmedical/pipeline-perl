...
sub runFilterVariants {
    ...
    $command .= "-S $opt{PIPELINE_PATH}/$opt{FILTER_SCALA} -R $opt{GENOME} -V $opt{OUTPUT_DIR}/$runName\.raw_variants.vcf -O $runName -mem $opt{FILTER_MEM} -nsc $opt{FILTER_SCATTER} -mode both ";
